...
- Latest release: **v0.2.4** (in-game integration: the companion mod's focus bridge, a "Show in game" button, follow the save the game is actually playing, and the Hyprland Lua-dispatch raise fallback). **v0.3.0** is reserved for when the companion mod is complete; until then the in-game integration work ships as incremental **0.2.x** releases.
